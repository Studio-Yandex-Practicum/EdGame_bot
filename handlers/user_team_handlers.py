...
@child_team_router.message(F.text.in_([BUTTONS['RU']['join_team'],
...
@child_team_router.callback_query(
    JoinTeam.team_chosen, F.data == 'team:info')
...
async def process_choose_team(query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if len(team.users) >= team.team_size:
            ...
            await query.message.edit_text(
                msg,
                reply_markup=team_full_keyboard(language))
            return
        await state.set_state(JoinTeam.join_team)
        await query.message.edit_text(
            msg,
            reply_markup=join_team_keyboard(language))
    except KeyError as err:
        logger.error(
...
async def process_join_team(query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logger.error(
            f'Ошибка при присоединении к команде: {err}')
...
async def process_become_captain(query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await query.message.edit_text(
            msg,
            reply_markup=leave_captain_position_keyboard(language))
    except KeyError as err:
        logger.error(
            f'Ошибка в ключевом слове при выборе стать капитаном: {err}')
    except Exception as err:
        logger.error(
            f'Ошибка при выборе стать капитаном: {err}')


@child_team_router.callback_query(F.data == 'leave_captain_position')
async def process_leave_captain_position(query: CallbackQuery,
                                         state: FSMContext):
    ...
    try:
        ...
        await query.message.edit_text(
            msg,
            reply_markup=become_cap_or_leave_team_keyboard(language))
    except KeyError as err:
        logger.error(
            f'Ошибка в ключевом слове при уходе с позиции капитана: {err}')
    except Exception as err:
        logger.error(
            f'Ошибка при уходе с позиции капитана: {err}')
